...
/**
 * Adds an "is-scrolled" class to the navbar once the page has
 * been scrolled past a small threshold, and removes it again
 * near the top. This is what turns the floating capsule shape
 * into a full-width bar once it's "stuck", matching the
 * reference clinic site's navbar behavior.
 *
 * The navbar itself is `position: fixed`, which is what
 * actually keeps it pinned to the top of the viewport at all
 * times while scrolling (see components.css — `position: sticky`
 * was not holding on this site, so `fixed` is used instead for
 * a guaranteed-to-stick result). Because `fixed` takes the
 * navbar out of the normal page flow, this function also
 * measures its real rendered height and reserves that same
 * amount of space at the top of the page (via `body`
 * padding-top) so content never renders underneath it.
 *
 * Re-runs on every page since renderHeader() rebuilds the
 * navbar markup fresh each time; this is called after
 * initHeaderAndFooter() in the DOMContentLoaded handler below,
 * so the navbar element already exists.
 */
function initNavbarScrollState() {
  ...
  function updateNavbarSpacing() {
    ...
    document.body.style.paddingTop = `${Math.ceil(rect.bottom)}px`;
  }
  ...
  // Set the correct state immediately (e.g. if the page loads
  // already scrolled down from a same-page anchor link).
  ...
  // The navbar's rendered height can change on resize (tablet
  // and mobile breakpoints use different padding/logo sizes),
  // and once web fonts / the logo image finish loading.
  window.addEventListener("resize", updateNavbarSpacing);
  window.addEventListener("load", updateNavbarSpacing);
